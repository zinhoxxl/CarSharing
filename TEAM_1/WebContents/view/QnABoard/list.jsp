...
<meta charset="UTF-8">
<script>
//현재 sessionId 얻을 수 없음으로 작동안함
function checkForm(){
	console.log('${sessionScope.sessionId}');
	if(${sessionScope.sessionId==null}){
		$('#myModal').modal('show');
	}else{
		location.href="./QnAWriteForm.go?id=${sessionScope.sessionId}&pageNum=${pageNum}&items=${items}&text=${text}";
	}	
}
</script>
<style>
  ...
  <form action="<c:url value="QnABoardListProcess.car"/>" method="post">  
   ...
   <c:if test="${not empty qnaList }">
     <c:forEach items="${qnaList}"  var="qna">          
		...
		<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-return-right" viewBox="0 0 16 16">
         ...
         <a href="./QnAViewProcess.car?firstNum=${firstNum}&lastNum=${lastNum}&num=${qna.num}&pageNum=${pageNum}&items=${items}&text=${text}" style="font-family: 'Do Hyeon', sans-serif;">${qna.subject}</a></td>
         <td>${qna.reg_date}</td>
         <td>${qna.readCount}</td>
   ...
   <div align="left">
      <table>
        ...
        <a href="./QnAWriteForm.car?id=hong&pageNum=${pageNum}&items=${items}&text=${text}" class="btn btn-primary">&laquo;글쓰기</a></td>
        </tr>
      </table>
   </div>
  </form> 
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" onclick='javascript:location.href="./QnAWriteForm.go?id=${sessionScope.sessionId}&pageNum=${pageNum}&items=${items}&text=${text}"'>예</button>
      </div>
    </div>
  </div>
</div>
