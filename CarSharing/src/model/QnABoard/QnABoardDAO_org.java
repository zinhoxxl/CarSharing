...
public class QnABoardDAO_org {
	//1.자신타입의 static 필드 선언
	private static QnABoardDAO_org instance;
	//2.default생성자를 private로 선언
	private QnABoardDAO_org(){}
	public static QnABoardDAO_org getInstance() {
		if(instance==null) instance = new QnABoardDAO_org();	
		return instance;
	}
		  ...
		  if((items==null && text==null)||(items.length()==0 || text.length()==0)) {//검색 조건이 파라미터로 넘어오지 않은 경우
			sql = " select c.* from "
					...
					+ "(select b.num num, "
				  	...
				  	+ " order by ref desc, re_step asc) a, (SELECT @ROWNUM:=0) AS R) c "
				  	+ " where rowNum limit ?,?";
	...
	public void insertQnA(QnABoardDTO qna) {
			...
			try { //신규글 등록 처리
				...
				if(qna.getRef()==0) {
					...
					pstmt.setString(5, qna.getIp());
					pstmt.setInt(6, curNum+1);
	...
	public String getLoginNameById(String id) {
		  ...
		  try {
			    ...
			    if(rs.next()) {
			    	name=rs.getString("writer");//rs.getString(칼럼명)
			    }
